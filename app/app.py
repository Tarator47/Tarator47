    ...
    if request.method == 'POST':
        ...
        if 'play' in request.referrer:
            ...
            if '/input_play' in request.referrer:
                ...
                # is_solved will be "None" since
                # the sudoku is not completed
                # input_array is initialized to play_list,
                # which contains the starting numbers of a sudoku
                # play_array refers to the compiled 2D list after
            ...
            # (when the user checks their sudoku)
            elif '/play' in request.referrer:
                # renders the play.html file
                # is_solved is initialized to a boolean
                # of whether the completed sudoku is valid
                # check_array is initialized to a 2D list
                # of booleans that signify if each cell is valid
                # input_array is initialized to play_list,
                # which contains the starting numbers of a sudoku
                # play_array refers to the compiled 2D list after
                # a user has finished playing the sudoku
                return render_template(
                    'play.html', is_solved=s.check_grid(),
        ...
        # if the POST request is from the menu page
            ...
            # if a Sudoku hasn't already been generated,
            # create one of the specified difficulty
            if not has_gen:
                g = Generate(content)
...
def solution():
    ...
    if request.method == 'POST':
        ...
        # build 2D list from inputted numbers in order to initialize
        # Sudoku object
        for i in range(9):
            row_list = []
            for j in range(9):
        ...
        # Create the solved Sudoku based off of the numbers
        # inputted by the user in '/input_solve'
        s = Sudoku(sudoku_grid_list)
        s.solve()
